from carte_eleve import * #Completer
...
def tour(f1,f2) :
    ...
    c1 = f1.enfile() #Completer
    c2 = f2.enfile() #Completer
    res = comp(c1,c2)
    if res == -1 :
        f2.enfile(c1) #Completer
        f2.enfile(c2) #Completer
        print('Joueur 2 a gagné le tour')
    elif res == 1 :
        f1.enfile(c1) #Completer
        f1.enfile(c2) #Completer
        print('Joueur 1 a gagné le tour')
    else :
        print('Bataille')
        if f1.taille >=3 and  f2.taille >=3 : #A compléter
            bataille(i,j,f1,f2)
        else :
            ...
            if f1 < 3 : # A compléter
                while not f2.est_vide(): # A compléter
                    f1.enfile(f2.defile) # A compléter
            else :
                while not f1.est_vide(): # A compléter
                    f2.enfile(f1.depile) # A compléter
...
def bataille(c1,c2,f1,f2) :
    ...
    bataille_tab= []
    while comp(c1,c2) == 0:
        bataille_tab.append(c1) # A compléter
        bataille_tab.append(c2) # A compléter
        bataille_tab.append(f1.defile()) # A compléter
        bataille_tab.append(f2.defile()) # A compléter
        c1 = f1.defile() #Completer
        c2 = f2.defile() #Completer
    if comp(c1,c2) == 1 :
        for carte in bataille_tab :
            f1.enfile(carte)#Completer
        f1.enfile(c1)#Completer
        f1.enfile(c2)#Completer
        print('Joueur 1 a gagné le tour')
    if comp(c1,c2) == -1 :
        for carte in bataille_tab :
            f2.enfile(carte) #Completer
        f2.enfile(c1)#Completer
        f2.enfile(c2)#Completer
        print('Joueur 2 a gagné le tour')     
